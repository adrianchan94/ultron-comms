...
function runCoordinator() {
    ...
    store
        ...
        .then(() => {
        // eslint-disable-next-line no-console
        console.log(`[ultron-comms] coordinator ready peerId=${store.peerId} coordinator=${store.coordinatorPort} isCoordinator=${store.isCoordinator}`);
    })
        .catch((err) => {
        // eslint-disable-next-line no-console
...
switch (command) {
    ...
    case "kev-bridge": {
        ...
        void import("./bridges/kev/bridge.js").then(({ runKevBridge }) => {
            void runKevBridge().catch((err) => {
                // eslint-disable-next-line no-console
                console.error(`[kev-bridge] fatal: ${err?.message ?? err}`);
                process.exit(1);
            });
        });
        break;
    ...
    default:
        console.log("Usage: agent-comms [setup|status|remove|bridge <id>|chat|send|dm|rooms|agents|read]");
        process.exit(1);
}
// ---------------------------------------------------------------------------
// Bridge runner
...
function runBridge(id) {
    ...
    if (!bridge) {
        console.error(`Unknown bridge: ${id}. Available: ${Object.keys(bridges).join(", ")}`);
        process.exit(1);
    }
    Promise.resolve(bridge.run()).catch((err) => {
        console.error(err);
        process.exit(1);
    });
}
...
function runChat(args) {
    const { name } = parseNameArg(args);
    runTui(name).catch((err) => {
        console.error(err);
        process.exit(1);
    });
}
function runUserCli(command, args) {
    const { name, rest } = parseNameArg(args);
    runCli(command, rest, name).catch((err) => {
        console.error(err);
        process.exit(1);
    });
}
...
function removePi() {
    ...
    const extensions = settings.extensions;
    if (extensions) {
        settings.extensions = extensions.filter((e) => {
