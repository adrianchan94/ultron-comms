 ...
 */
import { createWireServer, createWireConnection, } from "./transport.js";
import { nanoid } from "./nanoid.js";
import { CommsError } from "./store.js";
// ---------------------------------------------------------------------------
// Constants
// ---------------------------------------------------------------------------
const DEFAULT_COORDINATOR_PORT = Number.parseInt(process.env.ULTRON_COMMS_PORT ?? "19876", 10);
...
const COORDINATOR_BIND_HOST = process.env.ULTRON_COMMS_BIND ?? (COORDINATOR_HOST === "127.0.0.1" ? "127.0.0.1" : "0.0.0.0");
...
export class MeshStore {
    ...
    connectToCoordinator() {
        return new Promise((resolve, reject) => {
            const socket = createWireConnection({ port: this.coordinatorPort, host: COORDINATOR_HOST }, () => {
                ...
                socket.on("data", (data) => {
                    ...
                    for (const item of items) {
                        if (isMeshMessage(item)) {
                            ...
                            else if (item.method === "auth_failed") {
    ...
    becomeCoordinator() {
        return new Promise((resolve, reject) => {
            ...
            this.coordinatorServer.listen(this.coordinatorPort, COORDINATOR_BIND_HOST, () => {
        ...
        socket.on("data", (data) => {
            ...
            for (const item of items) {
                ...
                if (item.method === "introduce") {
                    if (!authValid(item.authToken)) {
                        // eslint-disable-next-line no-console
                        console.warn(`[ultron-comms] auth_rejected peer=${item.peerId ?? "?"} from coordinator handshake`);
                        try {
                            socket.write(encode({ method: "auth_failed", reason: "bad token" }));
                        }
                        catch { /* ignore */ }
                        try {
                            socket.destroy();
    ...
    async handleIntroduction(socket, msg) {
        ...
        if (this.agents.size > 0 || this.rooms.size > 0) {
            ...
            try {
                await writeAsync(socket, encode({ method: "state_sync", state }));
            }
            catch { /* drop */ }
        }
            ...
            case "delivery": {
                ...
                if (patch.agentId === this.peerId && this.onDelivery) {
                    ...
                    if (patch.event.type === "room_message") {
                        await this.markRead(patch.event.message.id, this.peerId, patch.event.message.room);
                    }
    ...
    async connectToPeerData(peer) {
        ...
        return new Promise((resolve) => {
            const socket = createWireConnection({ port: peer.port, host: COORDINATOR_HOST }, () => {
                const buf = new MessageBuffer();
    ...
    async broadcastToDataConnections(msg) {
        ...
        for (const sock of this.coordinatorServerSockets) {
            writes.push(writeAsync(sock, data).catch(() => {
    ...
    async broadcastPatch(patch) {
        const msg = { method: "state_update", patch };
        ...
        if (this.coordinatorSocket) {
            try {
                await writeAsync(this.coordinatorSocket, encode(msg));
            }
            catch { /* drop */ }
        }
    }
    ...
    async notifyRoomsOfStatus(agentId, status) {
    ...
    findMessageSender(messageId, room) {
        if (room) {
